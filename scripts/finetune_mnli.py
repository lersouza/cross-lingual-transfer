...
"""
This script is extracted from Huggingface transformers' lib:
https://github.com/huggingface/transformers/blob/master/examples/text-classification/run_glue.py

I added some functionality manipulate the model lexical part
and did some code formatting.

The License header is preserved and so is available the link for
the original file.

Finetuning the library models for sequence classification
on GLUE (Bert, XLM, XLNet, RoBERTa, Albert, XLM-RoBERTa).
"""
...
class ModelArguments:
    ...
    freeze_lexical: bool = field(
        default=False, metadata={
            "help": "Indicates if the model lexical part should be freezed."
        }
    )
    lexical: str = field(
        default=None, metadata={
            "help": "Path to pretrained lexical to use"
        }
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to
        # a json file, let's parse it to get our arguments.
    ...
    if model_args.lexical:
        load_pretrained_lexical(model, model_args.lexical)

    if model_args.freeze_lexical:
